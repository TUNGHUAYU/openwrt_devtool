...
test_action_list_prints_new_and_modified_packages(){
    DEVTOOL_DIR="/repo"
    NEW_PKG_LIST="/repo/workspace/developing/FEEDS/feed_devtool/new_pkg"
    MOD_PKG_LIST="/repo/workspace/developing/PACKAGES/feeds/base/modified_pkg"
    ...
    assert_contains "${output}" "workspace/developing/FEEDS/feed_devtool/new_pkg" &&
    assert_contains "${output}" "modified_pkg" &&
    assert_contains "${output}" "modify" &&
    assert_contains "${output}" "workspace/developing/PACKAGES/feeds/base/modified_pkg"
}
...
test_action_list_uses_compact_row_spacing(){
    ...
    [[ "${output}" == $'\n|No.'* ]] &&
    [[ "${output}" == *$'PKG-PATH'*$'\n\n|1  '* ]] &&
    [[ "${output}" == *$'workspace/developing/FEEDS/feed_devtool/new_pkg'*$'\n|2  '* ]] &&
    [[ "${output}" != *$'workspace/developing/FEEDS/feed_devtool/new_pkg'*$'\n\n|2  '* ]]
}
